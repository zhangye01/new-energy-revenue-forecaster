...
同时它会运行架构守护检查，确保新增模块有测试、脚本加载不遗漏、`app.js` 不继续膨胀。GitHub Actions 也会在推送 `main` 或提交 PR 时执行同一套检查。

## 项目文档

业务口径、模型说明、工程化准入基线、风险台账、验收用例和烟测清单已整理到 [docs/](./docs/README.md)。继续开发前建议先阅读业务交接包、工程化准入基线和验收用例，避免页面迭代偏离既定口径。
