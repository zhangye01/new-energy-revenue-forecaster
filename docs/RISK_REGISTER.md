...
| ID | 风险 | 等级 | 当前状态 | 处理策略 |
| --- | --- | --- | --- | --- |
| R-001 | `app.js` 仍然承担页面主控、事件绑定和部分渲染职责 | 中 | 6716 行，最大函数 86 行；项目列表展示、壳层事件处理器、全口径收入配置页表单保存、方案管理、批量更新、首页政策面板、历史页渲染、打印控制、场景可视化数据、对比页展示构造和对比选择规则已迁出 | 继续按页面拆控制器，但已低于阶段性 7000 行门槛 |
...
2. 多方案对比页已拆出选择规则，后续可继续迁出剩余图表编排和事件绑定。
